...
/*   Updated: 2022/05/23 23:46:32 by chduong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_map(t_cube *s, int i)
{
	...
	while (s->file[i] && is_empty(s->file[i]))
		i++;
	s->map_len = what_len_max(s, i);
	s->map_height = hm_lines(s, i);
	if (s->map_len > 50 || s->map_height > 50)
		return (write_error("maximum map size : 50x50\n"));
	s->map = malloc(sizeof(char *) * (s->map_height + 1));
	if (!s->map)
		return (write_error("malloc: memory allocation failed\n"));
	while (y < s->map_height)
	{
		s->map[y] = dup_with_space(s->file[i++], s->map_len);
		if (!s->map[y])
			return (write_error("malloc: memory allocation failed\n"));
		y++;
	}
